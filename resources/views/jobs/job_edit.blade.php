        ...
        <form action="{{ route('job.update', ['job' => $job]) }}" method="POST">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="location" value="{{ old('location', $job->location) }}" class="form-control"
                        id="location" placeholder="Branch location" required>
                    @error('location')
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="position" value="{{ old('position', $job->position) }}" class="form-control"
                        id="position" placeholder="Position" required>
                    @error('position')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                ...
                <select name="mode" id="mode" class="form-select" required>
                    ...
                    <option value="Full time" {{ old('mode', $job->mode) == 'Full time' ? 'selected' : '' }}>Full time
                    </option>
                    <option value="Part time" {{ old('mode', $job->mode) == 'Part time' ? 'selected' : '' }}>Part time
                    </option>
                </select>
                @error('mode')
                    <small class="text-danger">{{ $message }}</small>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="number" name="min_salary" value="{{ old('min_salary', $min_salary) }}"
                        class="form-control" id="min_salary" min="1" step="1" required>
                    @error('min_salary')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="col-md-6">
                    ...
                    <input type="number" name="max_salary" value="{{ old('max_salary', $max_salary) }}"
                        class="form-control" id="max_salary" min="1" step="1">
                    @error('max_salary')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="responsibility" id="responsibility" class="form-control" rows="3"
                    placeholder="Describe the key responsibility" required>{{ old('responsibility', $job->responsibility) }}</textarea>
            ...
            <div class="mb-3">
                ...
                <div id="requirements-container">
                    @if (old('requirements'))
                        ...
                        @endforeach
                    @else
                        @foreach ($jobRequirements as $index => $jobRequirement)
                            <div class="row mb-2 requirement-row">
                                <div class="col-md-8">
                                    <input type="text" name="requirements[]" class="form-control"
                                        value="{{ $jobRequirement->description }}" placeholder="Requirement" required>
                                </div>
                                <div class="col-md-3">
                                    <input type="number" name="weights[]" class="form-control" placeholder="Weight"
                                        value="{{ $jobRequirement->weight }}" min="1" required>
                                </div>
                                @if ($index != 0)
                                    <div class="col-md-1">
                                        <button type="button"
            ...
            <div class="mb-3">
                ...
                <textarea name="benefit" id="benefit" class="form-control" rows="3" placeholder="Describe what you offer"
                    required>{{ old('benefit', $job->benefit) }}</textarea>
                @error('benefit')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
